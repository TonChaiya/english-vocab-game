...
export async function GET(request: Request) {
  try {
    // getServerSession can be typed differently depending on NextAuth config.
    // Cast to any to avoid TypeScript issues here and validate runtime presence.
  ...
  let currentLevel = requestedLevel || progress.currentLevel
  ...
  // Normalize completed ids to string for comparisons
  const completedWordIds = (progress.completedWords || []).map((id: any) => String(id))

  // Also prepare ObjectId versions for DB queries that compare against _id
  const completedObjectIds = completedWordIds.map((id: any) => {
    try {
  ...
  const uncompletedWordsInStage = wordsInStage.filter((word: any) => !completedWordIds.some((id: any) => String(id) === String(word._id)))
  ...
  const playedWordsInStage = wordsInStage.filter((word: any) => completedWordIds.some((id: any) => String(id) === String(word._id)));
    ...
    if (words.length === 0 && !requestedStage) {
      ...
      const uncompletedWordsInNextStage = wordsInNextStage.filter((word: any) => !completedWordIds.some((id: any) => String(id) === String(word._id)))
        ...
        if (levelIndex < levels.length - 1) {
          ...
          const uncompletedNextLevelWords = nextLevelWords.filter((word: any) => !completedWordIds.some((id: any) => String(id) === String(word._id)))
    ...
    if (words.length === 0 && completedWordIds.length > 0) {
      ...
      const completedWords = await db.collection("words")
        .find({ _id: { $in: completedObjectIds.slice(0, 50) } })
        .toArray()
    ...
    for (const word of words) {
      ...
      wordsWithChoices.push({
        word,
        choices,
        completed: completedWordIds.some((id: any) => String(id) === String(word._id))
      })
    }
...
async function generateChoicesForWord(word: any, db: any) {
  ...
  while (uniqueChoices.length < 3) {
    const fakeTranslation: string = `ตัวเลือก ${uniqueChoices.length + 1}`
