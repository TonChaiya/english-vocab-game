...
export async function getWordForUser(userId: string, progress: any) {
  ...
  try {
    ...
    // Normalize completed word ids to string for consistent comparisons
    const completedWordIds = (progress.completedWords || []).map((id: any) => String(id))
    ...
    const allWordsPlayed = wordsInStage.every((word: any) => {
      const wordId = word._id.toString();
      return allPlayedWords.some((id: any) => {
    ...
    const uncompletedWords = wordsInStage.filter((word: any) => {
      const wordId = word._id.toString();
      return !allPlayedWordIds.some((id: any) => {
        const playedId = typeof id === 'string' ? id : id.toString();
        return playedId === wordId;
      });
    })
  ...
  // slice indices are 0-based while sequence numbers are 1-based
  const nextStageStartIndex = (nextStage - 1) * WORDS_PER_STAGE
  const nextStageEndIndex = nextStage * WORDS_PER_STAGE
    ...
    if (wordsInNextStage.length > 0) {
      ...
      // กรองคำศัพท์ที่ยังไม่ได้เล่น โดยใช้ playedWords
          const uncompletedWordsInNextStage = wordsInNextStage.filter((word: any) => {
        const wordId = word._id.toString();
        return !allPlayedWordIds.some((id: any) => String(id) === wordId);
      })
    ...
    if (currentIndex < levels.length - 1) {
      ...
      if (wordsInFirstStage.length > 0) {
        ...
        // กรองคำศัพท์ที่ยังไม่ได้เล่น โดยใช้ playedWords
        const uncompletedWordsInFirstStage = wordsInFirstStage.filter((word: any) => {
          const wordId = word._id.toString();
          return !allPlayedWordIds.some((id: any) => {
            const playedId = typeof id === 'string' ? id : id.toString();
            return playedId === wordId;
    ...
    if (completedWordIds.length > 0) {
      // Use $sample to pick previously completed words. Convert to ObjectId when querying _id
      const completedObjectIds = completedWordIds.slice(0, 50).map((id: any) => {
        try {
      ...
      const randomCompletedWords = await db
        .collection("words")
        .aggregate([
          { $match: { _id: { $in: completedObjectIds } } },
          { $sample: { size: 5 } }
        ])
        .toArray()
...
export async function getNextWords(userId: string, currentWordId: string, count: number = 3, selectedLevel?: string, selectedStage?: number) {
  try {
    ...
    // If requesting many words (e.g. entire stage), use bulk loader to avoid repeated DB calls
    const words: any[] = [];
    if (count >= 20) {
      // load up to `count` words from the stage using getAllWordsInStage and slice
      const levelToUse = selectedLevel || progress.currentLevel
      const stageToUse = selectedStage || progress.currentStage || 1
      const allWordsResult = await getAllWordsInStage(userId, levelToUse, stageToUse)
      if (allWordsResult && allWordsResult.nextWords) {
        const combined = [{ word: allWordsResult.word, choices: allWordsResult.choices }, ...allWordsResult.nextWords]
        for (let i = 0; i < Math.min(count, combined.length); i++) {
          if (combined[i]) words.push(combined[i])
        }
      }
    } else {
      ...
      for (let i = 0; i < count; i++) {
        // Apply selected level/stage on first iteration
        if (i === 0 && (selectedLevel || selectedStage)) {
        ...
        const wId = String(wordResult.word._id)
        if (!currentTempPlayedWords.includes(wId)) currentTempPlayedWords.push(wId)
        if (!currentTempCompletedWords.includes(wId)) currentTempCompletedWords.push(wId)
