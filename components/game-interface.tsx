...
export function GameInterface({ initialWord, initialChoices, userId, progress, stats }: GameInterfaceProps) {
  ...
  // timestamp of last /api/stats fetch to throttle repeated calls
  const lastStatsFetch = useRef<number>(0)
  // Lock to avoid duplicate concurrent "load more" calls
  const isFetchingMore = useRef<boolean>(false)
  // Lock to avoid duplicate concurrent prefetchNextWord calls
  const prefetchInFlight = useRef<boolean>(false)
  ...
  const loadGameWords = useCallback(async (page = 1, append = false) => {
    // Prevent concurrent append loads
    if (append && isFetchingMore.current) {
      console.log('Skipping loadGameWords because another append is in progress')
      return
    }
    try {
  ...
  // Release append lock
  if (append) isFetchingMore.current = false
    }
  ...
  const prefetchNextWord = async (currentWordId: string) => {
    ...
    if (prefetchInFlight.current) {
      console.log('Prefetch already in flight, skipping')
      return null
    }
    ...
    try {
      ...
      const response = await fetch("/api/words/next", {
            ...
            body: JSON.stringify({
              userId,
              currentWordId,
        prefetchCount: 20, // โหลดคำศัพท์ล่วงหน้าเป็นชิ้น (ลดจาก 100 -> 20)
              level: levelParam || undefined,
              stage: stageParam || undefined
            }),
            signal, // ใช้ signal จาก AbortController
      ...
      if (data.progress) {
        const levelChanged = data.progress.currentLevel !== currentProgress.currentLevel
        const stageChanged = data.progress.currentStage !== currentProgress.currentStage

        setCurrentProgress(data.progress)

        // Throttle stats fetches: fetch when level/stage changed or level/stage complete
        // or at most once every 10s as a fallback.
        const now = Date.now()
        const shouldFetchStats = levelChanged || stageChanged || data.levelComplete || data.stageComplete || (now - lastStatsFetch.current) > 10000
        if (shouldFetchStats) {
      ...
      if (data.word) {
        // If nextWords buffer is small, prefetch another chunk in background
        prefetchNextWord(data.word._id)

        // If we have a local `gameWords` buffer and it's running low, load next chunk
        if (gameWords.length > 0) {
          const remaining = gameWords.length - (currentWordIndex + 1)
          if (remaining < 5 && hasMoreWords) {
            // Load next chunk (page + 1) but avoid concurrent loads
